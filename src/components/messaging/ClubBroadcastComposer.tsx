...
import { previewClubBroadcastAction, sendClubBroadcastAction } from "@/app/messages/actions";
...
export function ClubBroadcastComposer({ initialPreview, clubId, route, clubName }: { initialPreview: BroadcastPreview; clubId: string; route: string; clubName: string }) {
  ...
  function refreshPreview() {
    ...
    startTransition(async () => {
      try { const result = await previewClubBroadcastAction(clubId); if (live.current) { if (result.ok) setPreview(result.data); else setError(result.error); } }
  ...
  function submit(event: FormEvent) {
    ...
    startTransition(async () => {
      try {
        const result = await sendClubBroadcastAction({ ...attempt, clubId });
        ...
        router.replace(`${route}/${result.data.id}`); router.refresh();
      } catch { if (live.current) { setUncertain(true); setError("발송 결과를 확인하지 못했습니다. 같은 요청으로 다시 확인해 주세요."); } }
      finally { busy.current = false; }
    });
  }
  return <form onSubmit={submit} aria-label="회원공지 작성" className="space-y-4 rounded-xl border border-pul-border bg-white p-4 sm:p-6">
    <h1 className="text-2xl font-black">회원공지 작성</h1><p className="font-bold [overflow-wrap:anywhere]">{clubName}</p>
    <p className="text-sm leading-6 text-pul-muted">이 동호회의 현재 정상 회원에게 동호회 공지를 보냅니다. 개인 차단 여부와 관계없이 전달되며 답장은 받지 않습니다.</p>
    ...
    <p className="text-sm leading-6 text-pul-muted">동호회 운영진 합산 3분에 1회, 24시간에 20회까지 발송합니다. 같은 내용은 24시간 내 다시 보낼 수 없습니다.</p>
    {uncertain ? <p role="status">중복 발송을 막기 위해 본문과 요청을 유지했습니다. 같은 요청으로 기존 발송 결과를 확인합니다. 새로고침하면 작성 내용은 저장되지 않습니다.</p> : null}
    {error ? <p role="alert" className="text-red-700">{error}</p> : null}
    <button type="submit" className={`${messageButton} !bg-pul-point !text-white`} disabled={pending || !viewActive || !confirmed || !preview.canSend}>{pending ? "발송 중…" : uncertain ? "같은 요청 다시 확인" : "회원공지 발송"}</button>
  </form>;
}
