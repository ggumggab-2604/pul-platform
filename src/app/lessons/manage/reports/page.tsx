...
import { LessonInformationReportManagementPage } from "@/components/lessons/manage/LessonInformationReportManagementPage";
...
export const metadata: Metadata = {
  title: "레슨 정보 제보 운영",
  description: "회원이 접수한 레슨 정보 제보 확인 및 처리",
};
      ...
      <Container className="max-w-3xl px-3 py-12">
        <div className="rounded-2xl border border-pul-border bg-white p-7 text-center">
          ...
          <p className="mt-2 text-base leading-7 text-pul-muted">
            {loadFailed
              ? "잠시 후 다시 시도해 주세요."
              : "이 화면은 active 레슨 운영자만 이용할 수 있습니다."}
          </p>
          <Link
            href="/lessons"
...
export default async function LessonInformationReportManagementRoute() {
  const context = await getAuthenticatedSupabaseContext();
  if (!context) redirect(`/login?next=${encodeURIComponent("/lessons/manage/reports")}`);

  let page;
  try {
    page = await listLessonInformationReportsForManagement(
      context.supabase,
      "pending",
      30,
      0,
    ...
    return (
      <AccessMessage
        loadFailed={!(error instanceof LessonInformationReportError && error.code === "permission")}
      />
    );
  }
    ...
    <div className="min-h-screen bg-pul-page">
      <Container className="max-w-5xl px-3 py-5 pb-20 lg:py-9">
        <header className="mb-5 rounded-2xl border border-pul-border bg-white p-5 sm:p-6">
          ...
          <p className="mt-2 text-base leading-7 text-pul-muted">
            회원 제보를 확인해 처리 완료하거나 별도 조치 없이 종료합니다. 제보 처리만으로 공개 레슨 정보가 자동 변경되지는 않습니다.
          </p>
        </header>
        <LessonInformationReportManagementPage initialPage={page} />
      </Container>
    </div>
  );
}
