-- PUL 8-3B-2B-4D correction: atomically prepare an initial administrator
-- membership and appoint the club administrator in one service-role operation.
-- The public signature and typed return contract remain unchanged.
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    if v_context_action_code = 'role.appoint_initial_admin' then
...
drop trigger if exists trg_guard_club_admin_assignment_delete
on public.club_role_assignments;

create trigger trg_guard_club_admin_assignment_delete
before delete on public.club_role_assignments
for each row
when (old.role_code = 'club_admin')
execute function private.enforce_guarded_club_admin_assignment_mutation();

create or replace function private.execute_club_admin_absence_mutation(
  p_action_code text,
  p_club_id uuid,
...
declare
  ...
  v_membership_relationship_created boolean := false;
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'This operation is available only to the service role.' using errcode = '42501';
  end if;
  ...
  if not found then
    raise exception 'The administrator candidate does not exist in Supabase Auth.';
  end if;
  ...
  if not found then
    if p_action_code <> 'role.appoint_initial_admin' then
    ...
    v_target_membership_status := 'active';
    v_membership_relationship_created := true;
  elsif v_target_membership_status <> 'active' then
    if p_action_code = 'role.recover_missing_admin' then
      raise exception '정상 활동 중인 회원만 새 회장이 될 수 있습니다.';
    end if;

    raise exception 'A suspended or left membership cannot be used for club administrator appointment.';
  end if;
  ...
  if v_member_assignment_id is null then
    ...
    values (
      ...
      'club_member',
      case
        when p_action_code = 'role.appoint_initial_admin' then null
        else p_operator_user_id
      end
    )
  ...
  values (
    ...
    case
      when p_action_code = 'role.appoint_initial_admin' then 'system'
      else 'operator'
    end,
    p_action_code,
    ...
    case
      when p_action_code = 'role.appoint_initial_admin' then 'initial_club_admin_appointment'
      else v_reason
    end,
    (
      case
        when p_action_code = 'role.appoint_initial_admin' then
          pg_catalog.jsonb_build_object(
            'declared_operator_user_id', p_operator_user_id,
            'execution_actor_type', 'service_role_system',
            'membership_relationship_created', v_membership_relationship_created
          )
        else
          pg_catalog.jsonb_build_object(
            'operator_user_id', p_operator_user_id
...
create or replace function public.appoint_initial_club_admin(
  p_club_id uuid,
  p_target_user_id uuid,
  p_operator_user_id uuid,
  p_request_id uuid,
...
comment on function private.execute_club_admin_absence_mutation(text, uuid, uuid, uuid, uuid, text) is
  'Internal service-role-only engine for atomic initial administrator membership preparation and appointment, plus unchanged emergency recovery, with global request replay and audit.';
...
comment on function public.appoint_initial_club_admin(uuid, uuid, uuid, uuid, text) is
  'Service-role-only atomic initial club administrator appointment. The declared operator scopes idempotency but the audit actor is recorded as the system service operation.';
